...
trait ScalaOps extends Base with OverloadHack { self: ArraysBase =>
...
//  def exit(status: Int): Rep[Nothing] = exit(toRep(status))
...
trait ScalaOpsExp extends ScalaOps with BaseExp { this: FunctionsExp with ArraysBase =>
...
trait ScalaGenScalaOps extends ScalaGenEffect  { this: ScalaOpsExp with ArraysBase =>
